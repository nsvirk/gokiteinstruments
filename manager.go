...
// Manager manages the instruments loaded from the Kite Connect API.
type Manager struct {
...
// NewManager creates a new Manager instance and loads the instruments from the Kite Connect API.
...
// isPreviousDayIST checks if the provided time is from the previous day or earlier in IST (Indian Standard Time).
...
// UpdateInstruments fetches the latest instruments from the Kite Connect API and updates the manager.
